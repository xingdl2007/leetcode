...
size_t hasher(const Key &k) {
  return std::hash<string>{}(k.data);
}
...
public:
  vector<vector<string>> groupAnagrams(vector<string> &strs) {
    ...
    std::for_each(table.begin(), table.end(), [&res](auto &vec) {
      res.push_back(std::move(vec.second));
    });
    return res;
  }
};
